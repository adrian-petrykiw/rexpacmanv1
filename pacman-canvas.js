	...
	function Game() {
		...
		this.init = async (state) => {
			...
			if (pinky === null || pinky === undefined) {
				var inky2 = 'https://metadata.degods.com/g/810.png';
				var clyde2 = 'https://metadata.degods.com/g/810.png';
				var blinky2 = 'https://metadata.degods.com/g/810.png';

				await db.collection("pacmannftpic").doc("currentpacmannftpic").get().then((doc) => {
					if (doc.exists) {
						const blinkyimage = doc.data().blinkyImageURL;
						blinky2 = blinkyimage;
						inky2 =  'img/inky.svg';
						clyde2 = 'img/clyde.svg';
						console.log("Injected " + blinky2);
					} else {
						// doc.data() will be undefined in this case
						inky2 = 'img/inky.svg';
						clyde2 = 'img/clyde.svg';
						blinky2 = 'img/blinky.svg';
						console.log("Injection failed!");
					}
				}).catch((error) => {
					console.log("Error getting document:", error);
				});
				console.log("Fetched Blinky");

				pinky = new Ghost(GHOSTS.PINKY, 7, 5, 'img/pinky.svg', 2, 2);
				inky = new Ghost(GHOSTS.INKY, 8, 5, inky2, 13, 11);
				clyde = new Ghost(GHOSTS.CLYDE, 10, 5, clyde2, 2, 11);
				blinky = new Ghost(GHOSTS.BLINKY, 9, 5, blinky2, 13, 0);
